...
プレイヤーたちは協力してバベルの塔の建設を目指します。しかし、その中には塔の完成を阻止しようとする「神の使い」と「狂信者」が潜んでいます。建築家チームは塔を完成させることができるのか、それとも妨害者たちが勝利を収めるのか。戦略と心理戦が鍵を握るゲームです。

## 2. 内容物

- **マニュアル QR コードカード**：1 枚
- **基盤カード**（3×3 のマス目が描かれたカード）：1 枚
- **資材ブロック**（10×10×10 mm の立方体ブロック）：25 個
- **親方トークン**：1 個
- **建築家トークン**：5 個
- **設計書カード**（塔の完成条件が書かれたカード）：13 枚
  - 事前に準備されたカード：10 枚
...
各カードには特定の効果や指示が書かれています。カードの種類とその効果をよく理解しておきましょう。

## 4. ゲームの準備

**神罰カードの枚数**：狂信者の人数と同じ  
**封印カードの枚数**：神罰カードの枚数 × 2 ＋ 1 枚  
**審議カードの枚数**：神罰カードの枚数と同じ
...
2. **特殊カードの準備**  
   「プレイヤー人数に応じた特殊カード一覧」に従い、神罰カード、封印カード、審議カードを取り分けます。
3. **場の準備**
   - 基盤カードを場の中央に表向きで置きます。
...
4. **山札の作成**
   ...
   - プレイヤーが 6 人以上の場合、残りの神罰カードと、プレイヤー人数 × 2 枚のカードを取り出し、合わせてシャッフルし、山札の上に戻します。
5. **親方の決定**  
...
1. **カードの選択**
   - 任命された建築家は手札から「プレイするカード」1 枚と「捨てるカード」1 枚を選びます。
...
- **親方の神罰カード処理**：
  - 親方は受け取ったカードの中に神罰カードがあれば、必ず自分の手札のカード 1 枚と交換します。この瞬間、親方は狂信者となります。
  - 親方が既に手札に神罰カードを持っている場合、その神罰カードを交換してはいけません。
  - 親方の手札が神罰カードのみの場合、全ての神罰カードを公開し、その時点で神の使い・狂信者チームの勝利となります（**神罰の確定**）。
...
- **塔の完成**：塔が設計書カードの条件を満たした場合、建築家チームの勝利となります。次に「封印フェーズ」に移行します。
...
1. **封印カードの使用宣言**
   - 封印カードを持つ建築家は名乗り出て、封印カードを使用することを宣言します。
   - 神の使いまたは狂信者は封印カードを使用できません。
2. **議論**
   - 全員で、誰が神の使いであるかを議論します。
   - 複数の封印カード使用者がいる場合、誰を封印するかについての相談はできません。それぞれが独自に決定します。
3. **封印対象の選択**
...
4. **勝敗の決定**
   ...
   - 神罰カードの枚数が封印カードの枚数未満であれば、**建築家チームの勝利**となります。
...
以上の修正により、ゲームのルールがより明確になり、円滑なゲーム進行が可能となります。
